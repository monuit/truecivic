...
type LabBillPageProps = {
  params: {
    identifier: string;
  };
};

export default async function LabBillInsightsPage({ params }: LabBillPageProps) {
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-4xl flex-col gap-6 bg-stone-950 p-6 text-white">
      <header className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-semibold">{billLabel} insights</h1>
          <p className="text-sm text-stone-400">
            Retrieval augmented notes captured from recent parliamentary records.
          </p>
        </div>
        <Link href="/labs" className="text-sm text-emerald-400 hover:underline">
          Back to labs
        </Link>
      </header>
