...
def get_twitter_share_url(url, description, add_plug=True):
    ...
    add_plug -- if True, will add a mention of truecivic.ca, if there's room """

    PLUG = ' (from truecivic.ca)'

    longurl = settings.SITE_URL + url

    try:
        shorten_resp_raw = urllib.request.urlopen(
            settings.BITLY_API_URL + urllib.parse.urlencode({'longurl': longurl}))
    ...
    return 'https://twitter.com/intent/tweet?' + urllib.parse.urlencode({'status': message})
