...
def try_postcode_first(request):
    ...
    try:
        member = ElectedMember.objects.get(
            end_date__isnull=True, riding=riding)
...
def postcode_to_riding(postcode: str) -> Riding | None:
    ...
    try:
        ...
        if riding:
            RidingPostcodeCache.objects.get_or_create(
                postcode=postcode, riding=riding, source='ourcommons')
            return riding
    except AmbiguousPostcodeException as e:
        raise
    except Exception:
    ...
    if edid:
        ...
        RidingPostcodeCache.objects.get_or_create(
            postcode=postcode, riding=riding, source='ec')
        return riding
...
def postcode_to_edid_represent(postcode):
    ...
    edid = [
        ...
        content.get('boundaries_concordance', []) +
        content.get('boundaries_centroid', [])
...
def postcode_to_edid_ec(postcode: str) -> int | None:
    ...
    raise AmbiguousPostcodeException(
        postcode=postcode, ec_url=urljoin(EC_POSTCODE_URL, redirect_url))


def postcode_to_riding_ourcommons(postcode):
    resp = requests.post(
        "https://www.ourcommons.ca/Members/search/members", json={"searchText": postcode})
    ...
    if len(content['currentMembers']) == 1:
        ...
        raise Exception(
            f"Cannot understand ourcommons response for postcode {postcode}")
...
def try_politician_first(request):
    try:
        pol = Politician.objects.get_by_name(request.GET['q'].strip(
        ), session=Session.objects.current(), saveAlternate=False, strictMatch=True)
        if pol:
...
class SearchFeed(Feed):
    ...
    def title(self, query):
        return '"%s" | truecivic.ca' % query

    def link(self, query):
        return "https://truecivic.ca/search/?" + urllib.parse.urlencode({'q': query.encode('utf8'), 'sort': 'date desc'})

    def description(self, query):
        return "From truecivic.ca, search results for '%s'" % query
